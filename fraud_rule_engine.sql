...
('TML05','timeline','Multiple sub-cases opened in a short window (quantity-limit churn)'),
...
LANGUAGE sql STABLE AS $$
    SELECT
        ...
        jsonb_build_object(
            ...
            'delta_pct', round(((t.patient_income_validated - t.patient_income_reported)
                                  / NULLIF(t.patient_income_reported,0))::numeric, 3)
        )
    ...
    WHERE t.patient_income_reported IS NOT NULL
      ...
      AND (t.patient_income_validated - t.patient_income_reported)
          > asnf_fraud.get_config_numeric('CON01','delta_pct') * NULLIF(t.patient_income_reported,0);
$$;
...
LANGUAGE sql STABLE AS $$
    ...
    WHERE t.patient_household_size_reported IS NOT NULL
      ...
      AND (t.patient_household_size_reported - t.patient_household_size_validated)
...
LANGUAGE sql STABLE AS $$
    SELECT
        ...
        jsonb_build_object(
            'subcase_count', COUNT(DISTINCT t.sub_case_id),
            'span_days', MAX(t.file_receipt_date_time::date) - MIN(t.file_receipt_date_time::date),
            'drug', MAX(t.drug)
        )
    FROM asnfdm.pcd_phi_consolidated_vw t
    WHERE t.sub_case_id IS NOT NULL
    GROUP BY t.case_id
    HAVING COUNT(DISTINCT t.sub_case_id) >= asnf_fraud.get_config_numeric('TML05','min_subcases')
       AND (MAX(t.file_receipt_date_time::date) - MIN(t.file_receipt_date_time::date))
